...
4. Merge into `~/.claude/settings.json`, keeping every other key: `"autoMemoryDirectory": "<home>/memory"`, the approved `cleanupPeriodDays`, and, if approved, the search rule appended to `permissions.allow` (create the list if missing; do not add it twice). Check the result with `python3 -m json.tool ~/.claude/settings.json`; a malformed settings file disables all of its settings.
