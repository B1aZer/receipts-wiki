...
On request, `python3 scripts/rw.py` gives `find <words>` (rank every note against the words, whatever its area; `--json` for machine-readable results), `history <note>` (every commit of one note), `recall <words>` (search the conversation archive), `resume [<session>|--cwd <path>]` (recover a lost session's decisions and next actions from the archive), `lint` (problems, warnings, every named rule over every note, and forgetting candidates; `--docs` also sweeps the folders memory names for documents no note names; `--json` for all of it), `build-index`, `record --agent <name>` (commit changes made outside the hooks, such as an import), and `install-git-hook` (a git pre-commit check in the memory repository that blocks any commit staging a secret value or a note without a name and description, including commits made by other agents or by hand). The `lint-review` skill walks through the lint report with you and changes only what you approve.
