...
- Setup offers to allow the search command (`Bash(python3 *receipts-wiki*/scripts/rw.py find *)` in `permissions.allow`). Claude Code asks before running any script, so without the rule every search needed an approval and `claude -p` sessions could not search at all; an `allowed-tools` line in the skill only moved the prompt to the skill itself. Both evals set the rule for the receipts-wiki arm, as setup leaves it.
