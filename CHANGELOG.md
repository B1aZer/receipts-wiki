...
- Drift eval (`evals/drift/`): four scenarios built from failures seen in a live memory home (a cursor that must follow the work, one fact written from two directories, a new note in an area with a hand-written index, recall from an unrelated directory), graded on the memory files and on answers. First full run (2026-09-17, 36 sessions, $5.42): both arms 14/14 file checks and 18/18 answers, receipts-wiki 72 model turns against 90 and 0 refused tool calls against 9. Every step names memory explicitly, so the eval does not yet test upkeep that the user does not ask for.
