...
## 9. Update 2026-09-22 — finding, warmth and drift (0.3.0-dev.4 … .10)

An audit of the author's live memory home, 215 notes at the time, found three failures that the design had no answer for. Each fix below was built against one of them, and the evidence is in `CHANGELOG.md`.

**What the audit found.** The session-start cursor was a day out of date: it called a pull request "not coded" that had been open since the previous morning. Thirty-two notes were listed by no index at all, because their area index was hand-written and never regenerated. Notes had grown to 43 KB. And three of the four sessions working one workstream had started in an unrelated directory, so the index they loaded described someone else's work.

**Finding notes, whatever the directory** (`rw.py find`, `skills/find`). Every active note is ranked against a few words with BM25 over the body plus a boosted name-and-description field, and the cursor that links the results is named alongside them. It is arithmetic over words already on disk: no embeddings, no service, 0.17 s over 203 notes. The literature supports the cheap end here — lexical retrieval is the strongest scalable default and overtakes agentic file exploration as a corpus grows ([arXiv:2607.26497](https://arxiv.org/abs/2607.26497)), an agent with only keyword search reaches over 90% of vector-database RAG ([arXiv:2602.23368](https://arxiv.org/abs/2602.23368)), and adding dense retrieval pays least in exactly the name-heavy regime this memory is ([arXiv:2606.04194](https://arxiv.org/abs/2606.04194)).

**Warmth, not age** (`rwlib/warm.py`). Session start lists the notes read or changed in the last 30 days, newest first, under a character budget. Use, not age, because this memory's oldest notes are standing rules that never stop applying while its newest are the live workstream; ageing them out would invert their value. Nothing is stored, moved or compressed, and a note off the list is reached exactly as before. This follows the tiered-memory result that eviction must weigh importance and usage rather than recency — plain oldest-out eviction permanently lost thousands of needed facts ([arXiv:2603.10032](https://arxiv.org/abs/2603.10032)) — and the decay work that modulates by relevance and access frequency rather than time alone ([arXiv:2601.18642](https://arxiv.org/abs/2601.18642)).

**Named checks after the turn** (`rwlib/checks.py`). After the turn's commit, the notes it wrote are checked and the findings reach the agent with its next message: `index-unlisted`, `note-too-big`, `cursor-too-long`, `link-misspelled`, `cursor-not-updated`, `doc-unnamed`. Timing is the point — selective, well-placed intervention beats both always-on injection and passive availability ([arXiv:2607.08716](https://arxiv.org/abs/2607.08716)) — and it is the only mechanism here that has demonstrably changed behaviour in the wild: on 2026-09-17 the stale-cursor check fired at 08:15 and the session rewrote its cursor nine minutes later.

**The sweep** (`rw.py lint`, `--docs`, `--json`). The same per-note rules run over every note, so memory that predates a check is not invisible; `orphan-note` is sweep-only because a new note has no inbound link yet. Orphan findings name the notes the orphan most likely belongs with, following the tools that suggest connections rather than flagging violations. `--docs` extends the sweep to the folders memory names and lists documents no note mentions. Borrowed from hyperresearch's 26-rule lint registry, and from its JSON-first CLI.

**What was measured and not built.** A memory-writing skill was designed and dropped. The drift eval (`evals/drift`) ran 60 sessions across two scenario sets, including one where no prompt mentions memory and the change arrives inside unrelated work: both arms scored 100% on file checks and answers, so there was no gap for a skill to close. That matches the skills literature, where 39 of 49 real coding skills gave zero improvement and three made results worse through guidance that conflicted with context ([arXiv:2603.15401](https://arxiv.org/abs/2603.15401)), against curated skills helping when they are narrow ([arXiv:2602.12670](https://arxiv.org/abs/2602.12670)). The failure that motivated the idea came from a session of over 1,200 messages, which these evals do not reproduce; the hook-side checks address it instead. Also deliberately not built: embeddings, automatic forgetting, contradiction resolution without a `Supersedes` line, and any recording of whole conversations to a service.
